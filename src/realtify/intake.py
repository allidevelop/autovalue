...
def _parse_areas(text: str) -> tuple[float | None, float | None]:
    # Площа може бути цілим числом («Загальна площа (кв.м): 48») — приймаємо і
    # ціле, і дробове. Кома-гард у гнучкому патерні, щоб не «перестрибнути» на
    # житлову площу (інакше total=living, як було на витягу Ростиславська 5А).
