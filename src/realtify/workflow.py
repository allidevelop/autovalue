    ...
    if cache_key and not manual_links and not force_research:
        ...
        if report_hits and len(report_hits) >= required:
            ...
            if tier in ("building", "complex"):
                # Курована база того ж будинку/ЖК — площа НЕ є критерієм відсіву: оцінювач
                # бере всі об'єкти дому, поправку на площу робить коефіцієнт (рядок 27
                # розрахунку). Інакше аналог того ж дому з більшою площею випадав за кепом
                # (Ростиславська 73.5→53%, Повітряних 98.4→132%), обраних ставало <required
                # і потік марно йшов у скрейп. Селектор все одно РАНЖУЄ за близькістю площі,
                # тож із >5 аналогів дому беруться 5 найближчих. Кеп знято лише для бази дому/ЖК.
                sel_overrides["max_area_delta_pct"] = 100000.0
            cfg = {
                **collection_cfg,
