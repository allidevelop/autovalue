...
def building_match_key(*, city: str | None, address: str | None) -> str:
    """Толерантний ключ дому для МАТЧИНГУ (реєстр дат, база козлів): на відміну від
    address_key, повністю знімає тип вулиці, область/район і дублі міста — щоб
    «Львівська обл., м. Львів, вулиця Хмельницького Б., будинок 207-В» збігалося з
    «Хмельницького Б., 207-В» (city=Львів). НЕ замінює address_key (кеш аналогів цілий)."""
    parts = [str(p) for p in (city, address) if p]
    raw = " ".join(parts).lower().replace("’", "'")
    # № квартири геть
    raw = re.sub(r"\b(?:кв\.?|квартира|апартаменти?|apt\.?)\s*№?\s*\d+\S*", " ", raw)
    # «Львівська обл.» / «… область» цілою фразою (прикметник + обл) — геть
    raw = re.sub(r"\b[а-яіїєґ'-]+\s+обл(?:асть)?\b\.?", " ", raw)
    raw = re.sub(r"\b(?:область|обл|район|р-н)\b\.?", " ", raw)
    raw = re.sub(r"\b(?:будинок|буд|місто|м)\b\.?", " ", raw)
    # тип вулиці — ПОВНІСТЮ геть (присутність/відсутність не має ламати матч)
    for _pat, _canon in _STREET_TYPE_CANON:
        raw = re.sub(rf"\b(?:{_pat})\.?(?=\W|$)", " ", raw)
    raw = re.sub(r"[^0-9a-zа-яіїєґ'\s]", " ", raw)
    raw = re.sub(r"(\d+)\s+([a-zа-яіїєґ])(?=\s|$)", r"\1\2", raw)
    # порядко-НЕзалежний набір токенів (дедуп + сорт): «Б. Хмельницького» ==
    # «Хмельницького Б.» (Богдана Хмельницького у різному порядку).
    toks = sorted({t for t in raw.split() if t})
    return "-".join(toks).strip("-")[:200]
